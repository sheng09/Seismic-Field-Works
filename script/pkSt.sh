...
HMSG="
Usage: pkSt.sh -D Directory -S stainfo.list -E evtinfo.list -A pretime(Second) -B subtime(Second) [-V]\n
\n
...
Finally, all the selected data will classified according to Sta name.\n
\n
...
i=0
mkdir SAC_ST 2>&- || rm SAC_ST/* -r -f 2>&-
...
while read stLINE; do
    ...
    stNm=`echo $stLINE    | awk '{print $2}'`

    FILENM=`echo $i $stNm | awk '{printf("SAC_ST/%04d%s",$1,$2)}'`
    echo "echo \#1\#$FILENM $VERBOSE" | sh
    echo "echo \#2\#$stLINE $VERBOSE" | sh
        ...
        if [[ ! -z $TrlT && $Dis > 30 && $Dis < 90 ]]; then
            ...
            if [[ $ANM == $BNM ]]; then
                if [[ ! -z `ls $DIR/$ANM*$kstDAS* 2>/dev/null` ]]; then
                    #Copy SAC_DATA to SAC_ST directory
...
q
EOF
                    #Add pre-name using Event Counts
                    CWD=`pwd`
                    cd $FILENM
                    TMP1=$ANM*$kstDAS*.1.sac
                    TMP2=`echo $iev $TMP1 | awk '{printf("%04d.%s",$1,$2)}'`
                    mv $TMP1 $TMP2


                    TMP1=$ANM*$kstDAS*.2.sac
                    TMP2=`echo $iev $TMP1 | awk '{printf("%04d.%s",$1,$2)}'`
                    mv $TMP1 $TMP2


                    TMP1=$ANM*$kstDAS*.3.sac
                    TMP2=`echo $iev $TMP1 | awk '{printf("%04d.%s",$1,$2)}'`
                    mv $TMP1 $TMP2
                    cd $CWD
                    echo "echo $evLINE $VERBOSE" | sh
                fi
            else
            #Merge records
                KMERGE=0
